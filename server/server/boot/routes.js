...
module.exports = function (server) {
    ...
    server.post('/login', function (req, res) {
        ...
        User.login(loginObj, 'user', function (err, token) {
            if (err) {
                var error = {
                    "error": {
    ...
    server.post('/request-password-reset', function (req, res) {
        var exists = false;
        User.find({ "where": { "email": req.body.email } }, function (err, users) {
            ...
            if (users && users.length) {
                var user = users[0];
                AccessToken.find({ "where": { "userId": user.id } }, function (err, accessTokens) {
                    if (err)
                        return res.sendStatus(404);
    ...
    server.post('/resetPassword', function (req, res) {
        if (req.body.token) {
            AccessToken.find({ "where": { "password_reset_otp": req.body.token } }, function (err, accessTokens) {
                if (err)
                    return res.sendStatus(404);
