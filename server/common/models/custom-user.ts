...
export  = function (Customuser:any) {
  Customuser.setup = function() {
    Customuser.base.setup.apply(this, arguments);

    this.beforeRemote('create', function(ctx: any, instance: any, next: any) {
      delete Customuser.validations.username
  
      var cluster_udise_id :any;
      ctx.args.data.username = ctx.args.data.username || ctx.args.data.phone_number;
      Customuser.app.models.School.find({where:{udise_id: ctx.args.data.school_udise_id}}, function(err: Error, schools:any){
        if (err) return console.log('School with udise_id ' + ctx.args.data.school_udise_id + ' not found!',err);
        ...
        Customuser.app.models.Cluster.find({where:{udise_id: cluster_udise_id}}, function(err: Error, clusters:any){
            if (err) return console.log('Cluster with udise_id ' + ctx.args.data.cluster_udise_id + ' not found!',err);
            if(clusters && clusters.length){
              var cluster = clusters[0]
              ctx.args.data.clusterId = cluster.id
              ctx.args.data.cluster_udise_id = cluster.udise_id
            }
            Customuser.app.models.Kp.find({where:{cluster_udise_id: cluster_udise_id}}, function(err: Error, kps:any){
              if (err) return console.log('KP with udise_id ' + ctx.args.data.kp_udise_id + ' not found!',err);
              if(kps && kps.length){
                var kp = kps[0];
  ...
  Customuser.on('resetPasswordRequest', function(info: any) {
